/* ============================================================================
   ...
   - Perzistencia v localStorage pod 'webovky_lang'
...
(function () {
  ...
  function load(lang) {
    ...
    var url = (window.WEBOVKY_I18N_PATH || './i18n/') + lang + '.json';
  ...
  function setLang(lang, persist) {
    ...
    return load(lang).then(function (dict) {
      applyDict(dict);
      window.dispatchEvent(new CustomEvent('webovky:lang-change', { detail: { lang: lang, dict: dict } }));
    }).catch(function (err) {
  ...
  // Expose
  window.Webovky = window.Webovky || {};
  window.Webovky.setLang = setLang;
  window.Webovky.getLang = function () { return localStorage.getItem(STORAGE_KEY) || detect(); };
  window.Webovky.SUPPORTED_LANGS = SUPPORTED;
})();
